...
var changeModalState = function changeModalState(state) {
  ...
  bindActionToNodes('click', windowShape, 'shape'); // ширина и высота в миллиметрах

  bindActionToNodes('input', windowHeight, 'height');
  bindActionToNodes('input', windowWidth, 'width'); // тип остекления балкона

  bindActionToNodes('change', windowType, 'type'); // теплое или холодное
...
var forms = function forms() {
  var allForms = document.querySelectorAll('form'),
      ...
      phoneInputs = document.querySelectorAll('input[name="user_phone"]'); // сообщения при отправке данных формы на сервер
  ...
  allForms.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); // создаем div-блок, в котором показываем сообщение

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status');
      item.appendChild(statusMessage);
      var formData = new FormData(item); // NEW дополняем данными из первого и второго модального окна
      ...
      sendPostData('assets/server.php', formData).then(function (response) {
...
var images = function images() {
  ...
  var imgPopup = document.createElement('div'); // секция, где расположены все изображения

  var workSection = document.querySelector('.works'); // создаем img-элемент большого изображения

  var bigImage = document.createElement('img'); // добавляем css-класс, который затемняет страницу

  imgPopup.classList.add('popup'); // модальное окно вставляем как дочерний элемент секции

  workSection.appendChild(imgPopup); // центрируем изображение по горизонтали и вертикали

  imgPopup.style.justifyContent = 'center';
  imgPopup.style.alignItems = 'center';
  imgPopup.style.display = 'none'; // внутрь модального окна вставляем большое изображение

  imgPopup.appendChild(bigImage); // обработчик события клика вешаем на секцию, а потом
  // проверяем, по какому именно изображению кликнули

  workSection.addEventListener('click', function (e) {
    e.preventDefault();
    var target = e.target; // если был клик по маленькому изображению
...
var modals = function modals() {
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    var trigger = document.querySelectorAll(triggerSelector),
        ...
        scrollWidth = calcScrollWidth(); // на странице есть две ссылки для открытия одного окна,
...
var tabs = function tabs(headerSelector, tabSelector, bodySelector, activeClass) {
  ...
  hideTabContent();
  showTabContent(); // при клике на контейнере переключателей выясняем, по какой ссылке
...
"use strict";
__webpack_require__.r(__webpack_exports__);
var timer = function timer(timerSelector, promoEndTime) {
  ...
  var getTimeRemaining = function getTimeRemaining() {
    ...
    var delta = Date.parse(promoEndTime) - Date.parse(new Date()),
  ...
  var setClock = function setClock() {
    ...
    timeInterval = setInterval(updateClock, 1000); // обновляем таймер сразу после загрузки страницы

    updateClock();

    function updateClock() {
      var delta = getTimeRemaining();
