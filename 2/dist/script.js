...
/* harmony import */ var _modules_phones__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modules/phones */ "./src/js/modules/phones.js");
/* harmony import */ var _modules_names__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./modules/names */ "./src/js/modules/names.js");
...
/* harmony import */ var _modules_scrolling2__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! ./modules/scrolling2 */ "./src/js/modules/scrolling2.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_calc__WEBPACK_IMPORTED_MODULE_6__["default"])('#size', '#canvas', '#option', '.promocode', '.calc-price');
  Object(_modules_filter__WEBPACK_IMPORTED_MODULE_7__["default"])();
  Object(_modules_pictureSize__WEBPACK_IMPORTED_MODULE_8__["default"])('.sizes .sizes-block');
  Object(_modules_accordion__WEBPACK_IMPORTED_MODULE_9__["default"])('#accordion .accordion-heading', '#accordion .accordion-block');
  Object(_modules_burger__WEBPACK_IMPORTED_MODULE_10__["default"])('.header .burger img', '.header .burger-menu'); // scrollup('.scroll-up');

  Object(_modules_scrolling2__WEBPACK_IMPORTED_MODULE_11__["default"])('.scroll-up');
...
var accordion = function accordion(triggerSelector, contentSelector) {
  var triggers = document.querySelectorAll(triggerSelector),
      contents = document.querySelectorAll(contentSelector); // для красивой анимации при показе ответа
  ...
  contents[0].style.display = 'block'; // обработчик события клика для каждого вопроса
...
"use strict";
__webpack_require__.r(__webpack_exports__);
var burger = function burger(burgerIconSelector, burgerMenuSelector) {
  var icon = document.querySelector(burgerIconSelector),
...
"use strict";
__webpack_require__.r(__webpack_exports__);
var calc = function calc(sizeSelector, canvasSelector, optionSelector, promoSelector, resultSelector) {
  ...
  var showSum = function showSum() {
    var size = parseFloat(sizeBlock.value),
        canvas = parseFloat(canvasBlock.value),
        option = parseFloat(optionBlock.value);
    var sum = Math.round(size * canvas + option);
...
var dragAndDrop = function dragAndDrop() {
   ...
   * элемента.
   */
  var fileInputs = document.querySelectorAll('[name="upload"]'); // браузер имеет свой собственный drag and drop, который запускается автоматически и
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (event) {
       ...
       */
      input.files = event.dataTransfer.files; // обрезаем оригинальное имя файла в операционной системе

      var orig = input.files[0].name; // оригинальное имя

      var parts = orig.split('.'); // имя и расширение

      var dots = parts[0].length > 10 ? '...' : '.';
      parts[0] = parts[0].length > 10 ? parts[0].substr(0, 10) : parts[0];
      var name = parts[0] + dots + parts[1]; // обрезанное имя
...
var filter = function filter() {
  ...
  var emptyCollection = document.querySelector('.portfolio-no');

  var show = function show(collection) {
...
/* harmony import */ var _services_request__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ../services/request */ "./src/js/services/request.js");
...
var forms = function forms() {
  var allForms = document.querySelectorAll('form'),
      ...
      allUploads = document.querySelectorAll('input[type="file"]'); // сообщения при отправке данных формы на сервер
  ...
  allUploads.forEach(function (item) {
    item.addEventListener('input', function () {
      var orig = item.files[0].name; // оригинальное имя
  ...
  var clearInputs = function clearInputs() {
    ...
    allUploads.forEach(function (item) {
      ...
      item.value = ''; // но мы для показа имени файла используем отдельный div
  ...
  allForms.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); // создаем контейнер, в котором показываем сообщение;
      // контейнер будет содержать абзац текста и картинку

      var statusContainer = document.createElement('div');
      item.parentNode.appendChild(statusContainer);
      statusContainer.style.display = 'none'; // внутрь контейнера помещаем абзац текста и картинку
      ...
      statusContainer.appendChild(statusImage); // скрываем форму с использованием классов анимации

      item.classList.add('animated', 'fadeOutUp');
      item.addEventListener('animationend', function () {
        // чтобы форма не занимала место в модальном окне
        item.style.display = 'none';
        statusContainer.style.display = 'block'; // показываем контейнер с использованием анимации
      ...
      var picture = item.querySelector('input[type="file"]');
      var handler = picture ? handlers.picture : handlers.message;
      var formData = new FormData(item);
      Object(_services_request__WEBPACK_IMPORTED_MODULE_7__["sendPostData"])(handler, formData).then(function (response) {
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var trigger = document.querySelectorAll(triggerSelector),
        ...
        close = document.querySelector(closeSelector); // на странице может быть несколько триггеров для окрытия одного и того
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        isOpenModal = true; // для модального окна подарка удаляем триггер
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      ...
      var documentHeight = document.documentElement.scrollHeight; // сколько уже прокручено от начала документа + ширина видимого окна браузера

      var currentHeight = window.pageYOffset + document.documentElement.clientHeight; // страница уже прокручена до конца или еще нет?

      var isScrollEnd = Math.abs(documentHeight - currentHeight) < 10; // пользователь не открывал модальных окон и страница прокручена до конца
...
var names = function names(selector) {
  var inputs = document.querySelectorAll(selector);
  inputs.forEach(function (input) {
    input.addEventListener('keydown', function (event) {
      var keys = ['Delete', 'Backspace', 'ArrowLeft', 'ArrowRight']; // разрешено использовать Delete, Backspace и стрелки
...
var phones = function phones(selector) {
  ...
  function mask(event) {
    ...
    var phoneValue = phonePattern.replace(/./g, function (a) {
      // если очередной символ является цифрой или подчеркиванием...
      var digitOrUnder = /[_0-9]/.test(a); // ...и мы еще не дошли до конца строки onlyDigits

      var notEndDigits = digitsIndex < onlyDigits.length; // ...то мы возвращаем очередной символ из onlyDigits
...
var pictureSize = function pictureSize(blockSelector) {
  ...
  var showImage = function showImage(block) {
    var image = block.querySelector('img'); // assets/img/sizes-1.png => assets/img/sizes-1-1.png

    image.src = image.src.replace('.png', '-1.png'); // скрыть размер холста и цену, которые внутри <p>
  ...
  var hideImage = function hideImage(block) {
    var image = block.querySelector('img'); // assets/img/sizes-1-1.png => assets/img/sizes-1.png

    image.src = image.src.replace('-1.png', '.png'); // показать размер холста и цену, которые внутри <p>
...
var scrolling2 = function scrolling2(scrollupSelector) {
  var scrollupButton = document.querySelector(scrollupSelector),
      allAnchorLinks = document.querySelectorAll('a[href^="#"]'); // кнопку прокрутки вверх показываем, когда страница прокручена на два экрана вниз
  ...
  allAnchorLinks.forEach(function (item) {
    item.addEventListener('click', function (event) {
      ...
      var allStepCount = Math.ceil(Math.abs(distWindowAnchor / oneStepPixels)); // текущий шаг, будем увеличивать значение, пока не достигнем allStepCount

      var curStepCount = 0; // за шаг мы смещаемся на oneStepPixels, но последний шаг может быть меньше

      var endStepPixels = Math.abs(Math.abs(distWindowAnchor) - oneStepPixels * allStepCount);
      requestAnimationFrame(smoothScroll);

      function smoothScroll() {
        curStepCount++; // на последнем шаге прокрутка может быть меньше, чем обычно

        var step = oneStepPixels;
...
/* harmony default export */ __webpack_exports__["default"] = (scrolling2);
...
var showMoreStyles = function showMoreStyles(buttonSelector, wrapperSelector) {
  var button = document.querySelector(buttonSelector),
      wrapper = document.querySelector(wrapperSelector);
  button.addEventListener('click', function () {
    Object(_services_request__WEBPACK_IMPORTED_MODULE_3__["getResource"])('assets/resource.php').then(function (response) {
      return createBlocks(response.styles);
...
var slider = function slider(slideSelector, direction, prevSelector, nextSelector) {
  var index = 1,
      paused = false;
  var items = document.querySelectorAll(slideSelector); // инициализация слайдера сразу после загрузки страницы

  showSlide(index); // запускаем автоматическую смену слайдов после загрузки страницы

  autoplay(); // когда мышь над слайдером — останавливаем автоматическую прокрутку
  ...
  try {
    ...
    prevButton.addEventListener('click', function () {
      nextPrevSlide(-1);
      items[index - 1].classList.remove('slideInLeft'); // следующий слайд плавно выезжает справа

      items[index - 1].classList.add('slideInRight');
    });
    nextButton.addEventListener('click', function () {
      nextPrevSlide(1);
      items[index - 1].classList.remove('slideInRight'); // следующий слайд плавно выезжает слева

      items[index - 1].classList.add('slideInLeft');
    });
  } catch (e) {} // console.log('У этого слайдера нет кнопок вперед и назад')
...
/*!************************************!*\
  !*** ./src/js/services/request.js ***!
  \************************************/
/*! exports provided: sendPostData, getResource */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "sendPostData", function() { return sendPostData; });
...
var sendPostData = function sendPostData(url, data) {
  var response;
  return regeneratorRuntime.async(function sendPostData$(_context) {
    while (1) {
    ...
    while (1) {
      switch (_context2.prev = _context2.next) {
        ...
        case 2:
          ...
          throw new Error("\u041D\u0435 \u0443\u0434\u0430\u043B\u043E\u0441\u044C \u043F\u043E\u043B\u0443\u0447\u0438\u0442\u044C \u043E\u0442\u0432\u0435\u0442 \u043E\u0442 ".concat(url, ", \u0441\u0442\u0430\u0442\u0443\u0441 ").concat(response.status));
